...
static inline void readU8Array(FILE* file, u8* ref_array, u32 arraySize)
{
	for (u32 i = 0; i < arraySize; i++)
	{
		ref_array[i] = readU8(file);
	}
}
...
static inline OS2 readOS2(FILE* file)
{
	OS2 os2 = {};
	os2.version = readU16(file);
	GRASSERT_DEBUG(os2.version == 2 || os2.version == 3 || os2.version == 4);
	os2.xAvgCharWidth = readI16(file);
	os2.usWeightClass = readU16(file);
	os2.usWidthClass = readU16(file);
	os2.fsType = readU16(file);
	os2.ySubscriptXSize = readI16(file);
	os2.ySubscriptYSize = readI16(file);
	os2.ySubscriptXOffset = readI16(file);
	os2.ySubscriptYOffset = readI16(file);
	os2.ySuperscriptXSize = readI16(file);
	os2.ySuperscriptYSize = readI16(file);
	os2.ySuperscriptXOffset = readI16(file);
	os2.ySuperscriptYOffset = readI16(file);
	os2.yStrikeoutSize = readI16(file);
	os2.yStrikeoutPosition = readI16(file);
	os2.sFamilyClass = readI16(file);
	readU8Array(file, os2.panose, 10);
	os2.ulUnicodeRange1 = readU32(file);
	os2.ulUnicodeRange2 = readU32(file);
	os2.ulUnicodeRange3 = readU32(file);
	os2.ulUnicodeRange4 = readU32(file);
	os2.useless = readU32(file);
	os2.fsSelection = readU16(file);
	os2.usFirstCharIndex = readU16(file);
	os2.usLastCharIndex = readU16(file);
	os2.sTypoAscender = readI16(file);
	os2.sTypoDescender = readI16(file);
	os2.sTypoLineGap = readI16(file);
	os2.usWinAscent = readU16(file);
	os2.usWinDescent = readU16(file);
	os2.ulCodePageRange1 = readU32(file);
	os2.ulCodePageRange2 = readU32(file);
	os2.sxHeight = readI16(file);
	os2.sCapHeight = readI16(file);
	os2.usDefaultChar = readU16(file);
	os2.usBreakChar = readU16(file);
	os2.usMaxContext = readU16(file);
	return os2;
}
