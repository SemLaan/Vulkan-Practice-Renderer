...
typedef struct MCData
{
    ...
    MCVertDarray* verticesDarray;    // Darray with all the vertices
...
void MCGenerateMesh()
{
	...
	mcdata->verticesDarray = MCVertDarrayCreate(VERTICES_START_CAPACITY, GetGlobalAllocator());
            ...
            for (u32 z = 0; z < mcdata->densityMapDepth - 1; z++)
            {
                ...
                if (!(cubeIndex == 0 || cubeIndex == 255))
                {
                    ...
                    for (i32 i = 0; triTable[cubeIndex][i] != -1; i++)
                    {
                        ...
                        // Adding the vertex
                        MCVertDarrayPushback(mcdata->verticesDarray, &vert);
                        numberOfVertices++;
                        ...
                        if (i % 3 == 2)
                        {
                            ...
                            vec3 edgeA = vec3_sub_vec3(mcdata->verticesDarray->data[numberOfVertices - 2].pos, mcdata->verticesDarray->data[numberOfVertices - 1].pos);
                            vec3 edgeB = vec3_sub_vec3(mcdata->verticesDarray->data[numberOfVertices - 3].pos, mcdata->verticesDarray->data[numberOfVertices - 1].pos);
                            vec3 normal = vec3_cross_vec3(edgeA, edgeB);

                            // Setting the normal for the three most recently added verts
                            mcdata->verticesDarray->data[numberOfVertices - 1].normal = normal;
                            mcdata->verticesDarray->data[numberOfVertices - 2].normal = normal;
                            mcdata->verticesDarray->data[numberOfVertices - 3].normal = normal;
                        }
                    }
                }
            }
    ...
    // Generate mesh
    mcdata->vertexBuffer = VertexBufferCreate(mcdata->verticesDarray->data, sizeof(*mcdata->verticesDarray->data) * numberOfVertices);
